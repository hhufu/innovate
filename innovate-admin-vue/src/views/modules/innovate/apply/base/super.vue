<template>
  <div class="mod-user">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="isAuth('innovate:project:update')" type="text" size="small" @click="statusAddOrUpdateHandle(scope.row.projectInfoEntity.projectId)">企业状态</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <status-add-or-update v-if="statusAddOrUpdateVisible" ref="statusAddOrUpdate" @refreshDataList="getDataList"></status-add-or-update>
...
<script>
  ...
  import StatusAddOrUpdate from './operation/status-add-or-update'
  ...
  export default {
    ...
    methods: {
      ...
      // 新增 / 修改
      statusAddOrUpdateHandle (id) {
        this.statusAddOrUpdateVisible = true
        this.$nextTick(() => {
          this.$refs.statusAddOrUpdate.init(id)
        })
      },
      // 删除
      deleteHandle (id) {
