<template>
  <div class="mod-user">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="applyIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="applyHandle(scope.row.projectInfoEntity.projectId)">分配工位号</el-button>
    ...
    <station-add-or-update v-if="stationVisible" ref="station" @refreshDataList="getDataList"></station-add-or-update>
...
<script>
  ...
  import StationAddOrUpdate from './operation/station-add-or-update'
        ...
        if (item.projectStatus === 0 || item.projectStatus === 1) {
          if (this.isAuth('innovate:project:apply:base')) {
            if (item.projectBaseApplyStatus !== null || item.projectBaseApplyStatus !== '') {
              if (item.projectBaseApplyStatus === 4 || item.projectBaseApplyStatus === 5 || item.projectBaseApplyStatus === 6) {
      ...
      retreatIsVisible (item) {
        if (item.projectStatus === 0 || item.projectStatus === 1) {
          if (this.isAuth('innovate:project:retreat')) {
            if (item.projectBaseApplyStatus !== null || item.projectBaseApplyStatus !== '') {
              if (item.projectBaseApplyStatus === 4) {
                let roleIdList = this.$store.state.user.roleIdList
                for (let roleIndex = 0; roleIndex < roleIdList.length; roleIndex++) {
                  if (roleIdList[roleIndex] === 5) {
                    return true
