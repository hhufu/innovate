<template>
  <div class="mod-user">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="applyIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="reviewAddOrUpdateHandle(scope.row.projectInfoEntity.projectId)">分配评委组</el-button>
          <el-button v-if="reApplyIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="reviewAddOrUpdateHandle(scope.row.projectInfoEntity.projectId)">重新分配评委组</el-button>
          <el-button v-if="retreatIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="retreatHandle(scope.row.projectInfoEntity)">不通过</el-button>
    ...
    <review-add-or-update v-if="reviewAddOrUpdateVisible" ref="review" @refreshDataList="getDataList"></review-add-or-update>
...
<script>
  ...
  import ReviewAddOrUpdate from './operation/review-add-or-update'
  ...
  export default {
    ...
    methods: {
      ...
      retreatHandle (item) {
        ...
        this.$nextTick(() => {
          this.$refs.retreat.init(item.projectId, 'project_base_apply_status', item.projectBaseApplyStatus)
        })
      },
      // 新增 / 修改
      reviewAddOrUpdateHandle (id) {
        this.reviewAddOrUpdateVisible = true
        this.$nextTick(() => {
          this.$refs.review.init(id, this.reApplyButtonVisible)
        })
      },
      retreatIsVisible (item) {
        if (item.projectStatus === 0 || item.projectStatus === 1) {
          if (this.isAuth('innovate:project:retreat')) {
            if (item.projectBaseApplyStatus !== null || item.projectBaseApplyStatus !== '') {
              if (item.projectBaseApplyStatus === 2) {
      ...
      reApplyIsVisible (item) {
        if (item.projectStatus === 0 || item.projectStatus === 1) {
          if (this.isAuth('innovate:project:apply:base')) {
            if (item.projectBaseApplyStatus !== null || item.projectBaseApplyStatus !== '') {
              if (item.projectBaseApplyStatus === 3) {
                let roleIdList = this.$store.state.user.roleIdList
                for (let roleIndex = 0; roleIndex < roleIdList.length; roleIndex++) {
                  if (roleIdList[roleIndex] === 5) {
