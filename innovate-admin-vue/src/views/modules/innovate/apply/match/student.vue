  ...
  <div class="mod-user">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('innovate:match:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
        <el-button v-if="isAuth('innovate:match:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <br v-if="isUpadate(scope.row.matchInfoEntity)"/>
          <el-button v-if="isUpadate(scope.row.matchInfoEntity)" type="text" size="small" @click="isUpdateHandle(scope.row.matchInfoEntity.matchId)">申请修改</el-button>
          <el-button v-if="isUpadate(scope.row.matchInfoEntity)" type="text" size="small" @click="updateHistoryHandle(scope.row.matchInfoEntity.matchId)">申请记录</el-button>
          <br v-if="applyMatchIsVisible(scope.row.matchInfoEntity)">
          <el-button v-if="applyMatchIsVisible(scope.row.matchInfoEntity)" type="text" size="small" @click="applyMatchHandle(scope.row.matchInfoEntity.matchId)">提交比赛申请</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 审批
      applyMatchHandle (id) {
        this.$confirm('此操作将使该项目进入不可修改状态，并提交给指导老师进入比赛审批流程，是否继续?', '提示', {
      ...
      addOrUpadate (item) {
        if ((item.projectMatchApplyStatus === 0) &&
          this.isAuth('innovate:match:update')) {
          return true
        }
      },
      isDelete (item) {
